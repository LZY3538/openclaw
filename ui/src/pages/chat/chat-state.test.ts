...
vi.mock("../../app/assistant-identity.ts", async (importOriginal) => ({
  ...
  loadLocalAssistantIdentity: mockLoadLocalAssistantIdentity,
}));
...
describe("resolveChatAvatarUrl", () => {
  ...
  it("returns null when the avatar is rejected for a non-missing reason", () => {
    mockLoadLocalAssistantIdentity.mockReturnValue({ avatar: null });
    const state = {
      sessionKey: "agent:main:main",
      assistantAvatar: "/avatar/main",
      assistantAgentId: "main",
    ...
    expect(resolveChatAvatarUrl(state)).toBeNull();
  });

  it("returns null when the avatar status is 'none' for any reason", () => {
    mockLoadLocalAssistantIdentity.mockReturnValue({ avatar: null });
    const state = {
      sessionKey: "agent:main:main",
      assistantAvatar: "/avatar/main",
      assistantAgentId: "main",
      assistantAvatarStatus: "none",
      assistantAvatarReason: "outside_workspace",
    } as unknown as ChatPageHost;
